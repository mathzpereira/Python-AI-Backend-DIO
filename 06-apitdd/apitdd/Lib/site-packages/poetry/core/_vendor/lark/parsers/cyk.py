...
class Rule:
    ...
    def __str__(self):
        return "{} -> {}".format(str(self.lhs), " ".join(str(x) for x in self.rhs))
...
class RuleNode:
    ...
    def __repr__(self):
        return "RuleNode({}, [{}])".format(
            repr(self.rule.lhs), ", ".join(str(x) for x in self.children)
        )
...
class UnitSkipRule(Rule):
    ...
    def __init__(self, lhs, rhs, skipped_rules, weight, alias):
        super().__init__(lhs, rhs, weight, alias)
