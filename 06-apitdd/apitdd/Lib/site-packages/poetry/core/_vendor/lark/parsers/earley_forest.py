...
class ForestToParseTree(ForestTransformer):
    ...
    def visit_symbol_node_in(self, node):
        super().visit_symbol_node_in(node)
        if self._on_cycle_retreat:
            return
        return node.children
    ...
    def visit_packed_node_in(self, node):
        self._on_cycle_retreat = False
        to_visit = super().visit_packed_node_in(node)
    ...
    def visit_packed_node_out(self, node):
        super().visit_packed_node_out(node)
...
class TreeForestTransformer(ForestToParseTree):
    ...
    def __init__(
        ...
        super().__init__(tree_class, dict(), prioritizer, resolve_ambiguity, use_cache)
