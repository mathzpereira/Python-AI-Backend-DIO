...
class ApplyTemplates(Transformer_InPlace):
    ...
    def template_usage(self, c):
        name = c[0].name
        args = c[1:]
        result_name = "{}{{{}}}".format(name, ",".join(a.name for a in args))
...
class FromPackageLoader:
    ...
    def __repr__(self):
        return "{}({!r}, {!r})".format(
            type(self).__name__, self.pkg_name, self.search_paths
        )
    ...
    def __call__(
        ...
        raise OSError("Cannot find grammar in given paths") from err
        ...
        for name, token_tree in term_dict.items():
            ...
            for exp in token_tree.find_data("value"):
                ...
                if isinstance(item, NonTerminal):
                    raise GrammarError(
                        "Rules aren't allowed inside terminals ({} in {})".format(
                            item, name
                        )
                    )
...
def _error_repr(error):
    if isinstance(error, UnexpectedToken):
        ...
        return "Unexpected token {!r}. Expected one of: {{{}}}".format(
            str(error.token), expected
        )
    else:
        return str(error)
...
def _get_mangle(prefix, aliases, base_mangle=None):
    def mangle(s):
        if s in aliases:
            ...
            if s[0] == "_":
                s = "_{}__{}".format(prefix, s[1:])
            else:
                s = "{}__{}".format(prefix, s)
    ...
    def _check_options(self, is_term, options):
        if is_term:
            if options is None:
                ...
                raise GrammarError(
                    "Terminal require a single int as 'options' (e.g. priority), got {}".format(
                        type(options)
                    )
                )
        else:
        ...
        for source in to_try:
            try:
                ...
                for name in gb._definitions:
                    if name in self._definitions:
                        raise GrammarError(
                            "Cannot import '{}' from '{}': Symbol already defined.".format(
                                name, grammar_path
                            )
                        )
            ...
            assert (
                False
            ), "Couldn't import grammar {}, but a corresponding file was found at a place where lark doesn't search for it".format(
                dotted_path
            )
        ...
        for name, d in self._definitions.items():
            ...
            for i, p in enumerate(params):
                if p in self._definitions:
                    raise GrammarError(
                        "Template Parameter conflicts with rule {} (in template {})".format(
                            p, name
                        )
                    )
                if p in params[:i]:
                    raise GrammarError(
                        "Duplicate Template Parameter {} (in template {})".format(
                            p, name
                        )
                    )
